...
__global__ void NNSmoothKernel ( float* pFieldIn, float* pFieldOut, size_t pitch, unsigned int halfwidth )
{
  // pitch is in bytes, figure out the number of elements for addressing memory locations in pFieldIn and pFieldOut
    ...
    unsigned indx = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned indy = blockIdx.y * blockDim.y + threadIdx.y;
    float mean = 0;
    for (int i = 0; i <= 2 * halfwidth; i++) 
        for (int j = 0; j <= 2 * halfwidth; j++) {
            int ind = (indy + i) * pitchels + indx + j;
            mean += pFieldIn[ind];
        }
    
    mean /= (2 * halfwidth + 1) * (2 * halfwidth + 1);

    pFieldOut[(indy + halfwidth) * pitchels + indx + halfwidth] = mean;
}
...
bool SmoothField ( float* pHostFieldIn, float *pHostFieldOut )
{
  ...
  const dim3 DimGrid = ((dataWidth - 2 * halfWidth) / DimBlock.x, (dataWidth - 2 * halfWidth) / DimBlock.y); 
